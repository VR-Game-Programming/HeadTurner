    ...
    void Update() {
        ...
        if (Physics.Raycast(cameraPosition, cameraDirection, out hit, lineLength)) {
            if (hit.collider.TryGetComponent<Renderer>(out var hitRenderer)) {
                if (hitRenderer.name == "Manager") {
                    Manager.SendMessage("HitTrack");
                }
                else if (hitRenderer.name == "StartArea") {
                    Manager.SendMessage("HitStartArea");
                }
                else if (hitRenderer.name == "EndArea") {
                    Manager.SendMessage("HitEndArea");
                }
            }
        }
    }
